...
#if defined(NDEBUG)
# error "LumoCash cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "LumoCashCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if (!nPrevHeight && nPrevHeight == 0) {
        	return 300000 * COIN;
    	} else if (nPrevHeight < 500) {
        	return 1 * COIN;
    	} else if (nPrevHeight >= 500 && nPrevHeight < 20000) {
        	return 40 * COIN;
    	} else if (nPrevHeight >= 20000 && nPrevHeight < 195480) {
        	return 30 * COIN;
    	} else if (nPrevHeight >= 195480 && nPrevHeight < 370960) {
        	return 27.5 * COIN;
    	} else if (nPrevHeight >= 370960 && nPrevHeight < 546440) {
        	return 25 * COIN;
    	} else if (nPrevHeight >= 546440 && nPrevHeight < 721920) {
        	return 22.5 * COIN;
    	} else if (nPrevHeight >= 721920 && nPrevHeight < 897400) {
        	return 20 * COIN;
    	} else if (nPrevHeight >= 897400 && nPrevHeight < 1072880) {
        	return 17.5 * COIN;
    	} else if (nPrevHeight >= 1072880) {
        	return 15 * COIN; 
    	}
}
...
void ThreadScriptCheck() {
    RenameThread("lumocash-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // LUMO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(LUMO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(LUMO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // LUMO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(LUMO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(LUMO): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        LumoCash Related Inventory Messages
