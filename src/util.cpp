...
const char * const BITCOIN_CONF_FILENAME = "lumocash.conf";
const char * const BITCOIN_PID_FILENAME = "lumocashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "lumocash" is a composite category enabling all LumoCash-related debug output
            if(ptrCategory->count(string("lumocash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LumoCashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LumoCashCore
    // Mac: ~/Library/Application Support/LumoCashCore
    // Unix: ~/.lumocashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LumoCashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LumoCashCore";
#else
    // Unix
    return pathRet / ".lumocashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty lumocash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override lumocash.conf
